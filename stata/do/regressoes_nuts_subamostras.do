* =============================================================================
* Regressões para as subamostras das NUTS 2
* =============================================================================
...
foreach d_var in "sim_mun" "sim_freg" {	
	foreach nutValue in "11" "15" "16" "17" "18" {		
		keep if nuts_2 == `nutValue'
		...
		if `nutValue' == 15 {
			local nutName = "Algarve"
		}
		
		if `nutValue' == 16 {
			local nutName = "Centro"
		}
		
		if `nutValue' == 17 {
			local nutName = "AMLisboa"
		}
		
		if `nutValue' == 18 {
			local nutName = "Alentejo"
		}
	
		regress `d_var' ippcc temp_med, vce(robust)
		eststo R_`nutName'_`i'
		local ++i
	...
	unicode convertfile "${user_directory}\stata\output\regressoes\temp_`d_var'.csv" "${user_directory}\stata\output\regressoes\nuts_`d_var'.csv", dstencoding(Windows-1252) replace
